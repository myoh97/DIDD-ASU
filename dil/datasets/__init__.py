...
def get_all_models():
    return [model.split('.')[0] for model in os.listdir('dil/datasets')
            if not model.find('__') > -1 and 'py' in model]


NAMES = {}
for model in get_all_models():
    mod = importlib.import_module('dil.datasets.' + model)
...
def get_dataset(args: Namespace, distill=False) -> ContinualDataset:
    """
    ...
    return NAMES[args.dataset](args, distill=distill)
