...
MEANS = {'cifar': [0.4914, 0.4822, 0.4465], 
         ...
         'seq-core50': [0.485, 0.456, 0.406],
         'seq-core50-s': [0.485, 0.456, 0.406],
...
STDS = {'cifar': [0.2023, 0.1994, 0.2010],
        ...
        'seq-core50': [0.229, 0.224, 0.225],
        'seq-core50-s': [0.229, 0.224, 0.225],
        }
MEANS['cifar10'] = MEANS['cifar']
MEANS['cifar100'] = MEANS['cifar']
MEANS['cifar10-c'] = MEANS['cifar']
MEANS['cifar100-c'] = MEANS['cifar']
STDS['cifar10'] = STDS['cifar']
STDS['cifar100'] = STDS['cifar']
STDS['cifar10-c'] = STDS['cifar']
STDS['cifar100-c'] = STDS['cifar']

MEANS['svhn'] = [0.4377, 0.4438, 0.4728]
MEANS['domain-net'] = [0.5071, 0.4867, 0.4408]
MEANS['mnist'] = [0.1307]
MEANS['perm-mnist'] = [0.1307]
MEANS['rot-mnist'] = [0.1307]
MEANS['fashion'] = [0.2861]
STDS['svhn'] = [0.1980, 0.2010, 0.1970]
STDS['domain-net'] = [0.2675, 0.2565, 0.2761]
STDS['mnist'] = [0.3081]
STDS['perm-mnist'] = [0.3081]
STDS['rot-mnist'] = [0.3081]
STDS['fashion'] = [0.3530]
...
class TensorDataset(torch.utils.data.Dataset):
    def __init__(self, images, labels, transform=None):
        self.images = images        # 반드시 CPU 텐서
        self.targets = labels       # 반드시 CPU 텐서
...
def transform_domainnet(augment=False, from_tensor=False, normalize=True):
    if not augment:
        aug = []
    else:
        aug = [transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip()]
        print("Dataset with basic DomainNet augmentation")
    ...
    if normalize:
        normal_fn = [transforms.Normalize(mean=MEANS['domain-net'], std=STDS['domain-net'])]
    else:
        normal_fn = []
...
def transform_pacs(augment=False, from_tensor=False, normalize=True):
    if not augment:
        aug = []
    else:
        aug = [transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip()]
        print("Dataset with basic PACS augmentation")
    ...
    if normalize:
        normal_fn = [transforms.Normalize(mean=MEANS['pacs'], std=STDS['pacs'])]
    else:
        normal_fn = []
...
def transform_core50s(augment=False, from_tensor=False, normalize=True):
    if not augment:
        aug = []
    else:
        aug = [transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip()]
        print("Dataset with basic CORe50-S augmentation")
    ...
    if normalize:
        normal_fn = [transforms.Normalize(mean=MEANS['seq-core50'], std=STDS['seq-core50'])]
    else:
        normal_fn = []
...
class ClassMemDataLoader():
    """Class loader with data on GPUs (CPU 병목 최소화 버전)
    """
    def __init__(self, dataset, batch_size, drop_last=False, device='cuda', joint=False):
        self.device = device
        self.batch_size = batch_size
        self.dataset = dataset

        # 1) 스택 + 1회 dtype 변환 + 메모리 포맷 정리
        self.data = torch.stack([d[0] for d in dataset])
        if self.data.dtype == torch.uint8:
            self.data = self.data.float().div_(255.0)
        if self.data.ndim == 4:
            self.data = self.data.contiguous(memory_format=torch.channels_last)

        # 2) 가능하면 GPU 상주(최상). 실패 시 CPU 핀 메모리.
        ...
        if joint or not hasattr(dataset, 'targets'):
            self.targets = torch.tensor([d[1] for d in dataset], device=device)
        else:
            self.targets = torch.as_tensor(dataset.targets, dtype=torch.long, device=device)
        ...
        self.nclass = int(self.targets.max().item()) + 1
        self.cls_idx = [(self.targets == c).nonzero(as_tuple=True)[0].tolist()
                        for c in range(self.nclass)]

        self.class_sampler = ClassBatchSampler(self.cls_idx, batch_size, drop_last=True)
        self.cls_targets = torch.arange(self.nclass, device=self.device).repeat(batch_size, 1).T

        # 런타임 변환 불필요(위에서 1회 변환 완료)
        self.convert = None

    def class_sample(self, c, ipc=-1):
        # GPU/CPU 텐서화된 인덱스 생성
        if ipc > 0:
            pool = torch.as_tensor(self.cls_idx[c],
                                   device=(self.device if self._gpu_resident else 'cpu'),
                                   dtype=torch.long)
            if ipc > pool.numel():
                pos = torch.randint(0, pool.numel(), (ipc,), device=pool.device)
                idx = pool[pos]
            else:
                idx = pool[torch.randperm(pool.numel(), device=pool.device)[:ipc]]
        else:
            idx_list = next(self.class_sampler.samplers[c])
            idx = torch.as_tensor(idx_list,
                                  device=(self.device if self._gpu_resident else 'cpu'),
                                  dtype=torch.long)

        # 데이터 선택 + 전송
        if self._gpu_resident:
            data = self.data.index_select(0, idx)
        else:
            cpu_batch = self.data.index_select(0, idx.cpu())
            if not cpu_batch.is_pinned():
                cpu_batch = cpu_batch.pin_memory()
            data = cpu_batch.to(self.device, non_blocking=True)

        # 타깃 생성(배치 크기 가변 대응)
        if ipc > 0 and ipc != self.batch_size:
            target = torch.full((idx.numel(),), c, dtype=torch.long, device=self.device)
        else:
            target = self.cls_targets[c]
        return data, target

    def sample(self):
        idx_list = next(self.iterator)
        idx = torch.as_tensor(idx_list,
                              device=(self.device if self._gpu_resident else 'cpu'),
                              dtype=torch.long)
        if self._gpu_resident:
        ...
        target = self.targets.index_select(0, idx.to(self.targets.device))
        return data, target
...
#     def class_sample(self, c, ipc=-1):
#         if ipc > 0:
#             if ipc > len(self.cls_idx[c]):
#                 # sample with replacement
#                 indices = np.random.choice(self.cls_idx[c], ipc, replace=True)
#             else:
#                 # random indices 
#                 indices = np.random.choice(self.cls_idx[c], ipc, replace=False)
#         else:
#             indices = next(self.class_sampler.samplers[c])
#         data = self.data[indices].to(self.device)
#         if self.convert:
#             data = self.convert(data)
